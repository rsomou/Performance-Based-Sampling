...
def get_initial_loaders(dataset, sampling_ratios):
    ...
    dataloaders['train'] = DataLoader(train_ds, batch_size=32, shuffle=True, pin_memory = True, num_workers = 2)
    dataset_sizes['train'] = len(train_ds)

    if 'valid' in dataset:
        valid_ds = ImageDataset(dataset, 'valid', transform=transform)
        dataloaders['valid'] = DataLoader(valid_ds, batch_size=32, shuffle=True, pin_memory = True, num_workers = 2)
